...
{
    public:
    Node_class( float theta_in , float dist_in)
    {
        double theta_rad=0;
        theta   = (theta_in * 90.f) / 16384.f;
        dist    = dist_in /4.0f;
        theta_rad = (theta*3.14159)/180;
        x_coo   = dist * cos(theta_rad);
        y_coo   = dist * sin(theta_rad);
    }
    ...
    float get_x_coo(void)
    {
        return x_coo;
    }

    float get_y_coo(void)
    {
        return y_coo;
    }
...
int main(int argc, const char * argv[]) {
   ...
   fd = serialOpen ("/dev/ttyS0", 9600);
    ...
    while (1) {
        sl_lidar_response_measurement_node_hq_t nodes[370];
        ...
        if (SL_IS_OK(op_result)) {
            ...
            for (int pos = 0; pos < (int)count ; ++pos) {
                ...
                buffer_ptr = new Node_class(nodes[pos].angle_z_q14 , nodes[pos].dist_mm_q2);
